...
```powershell
cd ~/Desktop
git clone https://github.com/HashiSEBot/SEWorkshop-Azure-Student
cd SEWorkshop-Azure-Student/terraform
code -r .
```
...
---
name: workstation-setup-11
First Look at Terraform Code
-------------------------
As part of setup for this workshop, we need to take a look at the *main.tf* and the *main.tf.completed* file
...
Using the Organization and Hostname written on the whiteboard by the instructor please replace ORGANIZATION_NAME & PTFE_SERVER in both files.

Lastly, please replace the student# with your student number for this workshop.

---
name: workstation-setup-12
First Look at Terraform Code
-------------------------
Your changes should look similar to this.

```terraform
terraform {
  backend "remote" {
    organization = "mycompany-workshop"
    hostname     = "ptfe.mycompany.workshop.hashidemos.io"
    token        = "PTFE_TOKEN"
    workspaces {
      name = "student99-workspace"
    }
  }
}
```

Now we can move on to getting a token to finish setup

---
name: tfe-setup
Create Terraform Enterprise API Token
-------------------------
The hostname in the last project is actually a URL, https://HOSTNAME

.center[![:scale 30%](images/tfe-login.png)]

Your username is: student#

Your password is: Ready2Terraform

---
name: tfe-setup-2
Go to TFE Token Page
-------------------------
In the top right, choose your user icon, and choose *User Settings*

.center[![:scale 50%](images/tfe-users.png)]
...
Generate a token
-------------------------
Use any description and choos *Generate Token*

.center[![:scale 50%](images/tfe-tokens.png)]

Copy the generated token to your *main.tf* and *main.tf.completed* files.

This will conclude the setup of your terraform for enterprise
...
* Configured Terraform Enterprise
]
...
When in your TFE UI, Choose your workspace, and choose variables.

.center[![:scale 80%](images/tfe-variables.png)]

This is your first view of enterprise variables. Feel free to set the prefix to something unique to you.
...
<br><br><br>
Your enterprise variables tab  should now look similar to this:

.center[![:scale 80%](images/tfe-variables2.png)]
...
resource "azurerm_resource_group" "hashitraining" {
  name     = "${var.prefix}-${var.workshop-prefix}-workshop"
...
resource "azurerm_virtual_network" "vnet" {
  name                = "${var.prefix}-${var.workshop-prefix}-vnet"
...
resource "azurerm_subnet" "subnet" {
  name                 = "${var.prefix}-${var.workshop-prefix}-subnet"
...
.contents[
In this chapter we:
* Learned about terraform state in enterprise
]
...
.section[
Chapter 7  
Terraform Enterprise Additional Features
]

---
name: pmr-configdesigner
